...
//Reposotory명 + Impl 형태로 인터페이스 이름 작성 (필수)
...
public class BookRepositoryImpl implements BookRepositoryExtension{
	...
	public List<Tuple> groupBy() {
		...
		List<Tuple> tuples = 
				queryFactory.select(book.category, book.bookAmt.max(), book.bookAmt.avg(), book.rentCnt.avg())
				.from(book)
	...
	public List<Book> dynamicQueryWithBook(BookDto bookDto) {
		...
		List<Book> books = 
				...
				.from(book)
				.where(bookAmtGoe(bookDto), rentCntLoe(bookDto)) //BooleanExpression은 Predicate 인터페이스를 구현
				.fetch();	
